    ...
    with src_engine.connect() as conn:
        ...
        if not pk_columns:
            ...
            if "PART_ID".lower() in all_columns:
                ...
                create_time_col = f', ct."CREATE_TIME" AS baseline_create_time'
                create_time_col_agg = f', COALESCE(MAX(t.baseline_create_time), 0) AS max_create_time'
            elif "TBL_ID".lower() in all_columns:
                create_time_table_join = f"JOIN {catalog_name}public.\"TBLS\" ct ON ct.\"TBL_ID\" = t.\"TBL_ID\""
                create_time_col = f', ct."CREATE_TIME" AS baseline_create_time'
                create_time_col_agg = f', COALESCE(MAX(t.baseline_create_time), 0) AS max_create_time'
            elif "SD_ID".lower() in all_columns:
                create_time_table_join = f"JOIN {catalog_name}public.\"PARTITIONS\" ct ON ct.\"SD_ID\" = t.\"SD_ID\""
                create_time_col = f', ct."CREATE_TIME" AS baseline_create_time'
                create_time_col_agg = f', MAX(t.baseline_create_time) AS max_create_time'
